        ...
        for i in range(len(clients_model_list)):
            ...
            if sum_parameters is None:  # 先初始化模型字典，主要是初始化key
                ...
                for key, var in local_parameters.items():
                    if 'norm' not in key and 'bn' not in key and 'sigma' not in key and 'downsample.1' not in key:
...
def Fedper_set_averaged_weights_as_main_model_weights(center_model,clients_model_list,weight_of_each_clients):
    sum_parameters = None  # 用来接所有边缘节点的模型的参数
    global_parameters = {}
    o=0
    cnt = len(center_model.state_dict().items())
    for key, var in center_model.state_dict().items():
        global_parameters[key] = var.clone()
