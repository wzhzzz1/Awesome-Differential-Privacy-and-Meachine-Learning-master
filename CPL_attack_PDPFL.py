...
plt.axis('off')
plt.savefig("./attack_image/sample_PDPFL.png")
plt.clf()

net = mnist_fully_connected(10).to(device)
net_PDPFL = mnist_fully_connected(10).to(device)
...
original_dy_dx_trimmed = original_dy_dx[:3]
'''
noise_std = 0.1
noise_mean = 0.0
...
for iters in range(300):
    def closure():
        ...
        dummy_pred = net_PDPFL(dummy_data)
        dummy_onehot_label = F.softmax(dummy_label, dim=-1)
        dummy_loss = criterion(dummy_pred, dummy_onehot_label)
        dummy_dy_dx = torch.autograd.grad(dummy_loss, net_PDPFL.parameters(), create_graph=True)  # faked数据得到的梯度

        grad_diff = 0
        for gx, gy in zip(dummy_dy_dx, original_dy_dx_trimmed):
...
for i in range(30):
    ...
    plt.axis('off')
plt.savefig("./attack_image/attack_result_PDPFL.png")
plt.show()
